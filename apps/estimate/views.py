...
from apps.estimate.forms import UploadImageForm, manageForm, descForm, companyCode
...
def index():
    ...
    try:
        ...
        pass    
    return render_template("estimate/index.html", form_com=form2, string2=finalHtml, form_desc = form1, list1 = manageList)

@est.route("/company", methods=["GET","POST"])
def routeCompany():
    form=companyCode()

    if form.validate_on_submit():
        return redirect(url_for("est.searchCompany",company_code = request.form['company']))
    return redirect(url_for("est.index"))

@est.route("/company/<company_code>", methods=["GET","POST"])
def searchCompany(company_code):
    form2=companyCode()
    global g_year, g_version, g_comp, g_mm, g_tomm
    form1 = descForm()
    if g_version[-3:] == '(P)' or g_version[-3:] == '(A)':
    ...
    # print(g_version[-3:], "확인부탁해요.", l_version, l_version2)
    # 현재 버전 (추정, 실적, 계획)을 조회하는 부분
    sql1 = queries.sum_IS_normal.where(and_(IncomeState.year == g_year, IncomeState.company==company_code, IncomeState.version == g_version, IncomeState.curr != 'EXW',IncomeState.mm>=g_mm,IncomeState.mm<=g_tomm)).order_by(IncomeState.mm)
    # 이외 부분은 실적(추정기간 앞), 이후는 계획을 넣고 마무리
    sql_act = queries.sum_IS_normal.where(and_(IncomeState.year == g_year, IncomeState.company==company_code, IncomeState.version == l_version, IncomeState.curr != 'EXW',IncomeState.mm<g_mm)).order_by(IncomeState.mm)
    sql_plan = queries.sum_IS_normal.where(and_(IncomeState.year == g_year, IncomeState.company==company_code, IncomeState.version == l_version2, IncomeState.curr != 'EXW',IncomeState.mm>g_tomm)).order_by(IncomeState.mm)
    manageList = specialThings.query.filter_by(year=g_year).all()
    ...
    return render_template("estimate/company_summary.html", form_com = form2, string2=finalHtml, form_desc = form1, company = company_code)
